extends RefCounted
## Disposable tutorial data. These strings are NOT account or blockchain secrets.
...
func authenticate(id: String, kind: String) -> bool:
	if stage != Stage.TITLE or id.is_empty() or kind not in ["google", "test"]:
		return false
	user_id = id
	login_kind = kind
...
func public_summary() -> Dictionary:
	return {"user_id": user_id, "login_kind": login_kind, "nickname": nickname, "address": address, "tutorial_complete": stage == Stage.COMPLETE}
